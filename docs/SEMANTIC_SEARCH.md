...
同一数据库快照、同一请求 → 结果顺序**逐位确定**（连发/重启均一致）。

**统一 tie-break 规则**（PR-04/PR-E 已实现，PR-E.1 审计锁定并补齐缺口）：

- 通道 SQL：lexical/tag/product `score DESC, shot_id ASC`；vector `distance ASC, shot_id ASC`
  （方向统一 distance，不与 similarity 混用）；browse `created_at DESC, id DESC`。
- 融合全序：`final_score ↓ → quality ↓ → 人工确认优先 → created_at ↑ → shot_id ↑`；
  RRF rank 赋值排序 `(-channel_score, shot_id)`。
- 二次排序（latest/duration/quality）与 usage-aware 重排（`final ↓ → base ↓ → shot_id ↑`）
  均以 shot_id 收尾；分页在全序列表上切片（page1+page2 ≡ 一次性 top_k，无重复无遗漏）；
  候选扩张每轮同键。
- 通道归并：`dict[shot_id]` 聚合 + 通道**串行**执行，不依赖 set 遍历序或并发完成序。

**PR-E.1 修复的三个非确定源**（不改任何分数公式形状与筛选语义）：

1. **LLM 查询解析缓存**（`CachedQueryParser`，仅包 mimo）：MiMo 对同一查询的
   `semantic_text` 改写与实体识别非确定 → 通道输入抖动 → 顺序/分数漂移。
   以 `sha256(查询文本)`+模型为键缓存首次 OK 解析结果（Redis，
   `SEARCH_PARSER_CACHE_TTL_SECONDS` 默认 7 天，0 禁用）；降级解析不固化；
   Redis 不可用静默直呼内层。fake/rulebased 解析器本身确定，不包缓存。
2. **suggestions 三条 SQL 补 `ORDER BY 归一名, id`**（LIMIT 无 ORDER BY 时
   行序随执行计划漂移）。
3. **usage 特征 days 量化为整天（floor）**：连续小数天使 recent/lru 调整随
   每次请求的 now 微变，同分候选在临界点排序翻转；整数天与"N 天没用"的产品
   语义一致，同一天内特征恒定，跨天推移是预期业务变化。公式形状/权重/上限未动。

**已知边界**（记录，不在本 PR）：文档嵌入在批处理中与单条嵌入存在浮点差异
（transformer padding 效应）——查询侧永远单条不受影响；文档侧重建/补嵌入时
分数可能变化，属"索引数据变化"而非请求间非确定。

回归锁定：`apps/api/tests/test_search_deterministic_order.py`（主动制造同分）；
E2E：`scripts/ci_pr_e1_search_determinism_e2e.py`（20 连发逐位 + 分页 + 重启）。
