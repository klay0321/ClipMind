...
def test_scan_full_lifecycle(session, tmp_path):
    ...
    counts1 = _scan_files(session, sd, run1, root)
    _mark_missing(session, sd.id, run1.id)
    ...
    counts2 = _scan_files(session, sd, run2, root)
    _mark_missing(session, sd.id, run2.id)
    ...
    counts3 = _scan_files(session, sd, run3, root)
    _mark_missing(session, sd.id, run3.id)
    ...
    _scan_files(session, sd, run4, root)
    missing = _mark_missing(session, sd.id, run4.id)
    assert missing == 1
    ...
    _scan_files(session, sd, run5, root)
    _mark_missing(session, sd.id, run5.id)
...
def test_scan_corrupt_marks_error(session, tmp_path):
    ...
    counts = _scan_files(session, sd, run, root)
