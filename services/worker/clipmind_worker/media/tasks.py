...
from clipmind_worker.auto_chain import ai_budget_exceeded, auto_request_ai
...
def _maybe_auto_ai(asset_id: int, settings: WorkerSettings) -> None:
    """AAP：拆镜头完成后自动入队 AI 打标（best-effort，独立短会话）。

    受 auto_ai_after_shots 开关与 ai_daily_budget 日预算护栏约束（仅自动
    路径；手动端点不受限）。失败只记日志，绝不影响拆镜头结果。
    """
    if not settings.auto_ai_after_shots:
        return
    try:
        with SessionLocal() as session:
            if ai_budget_exceeded(session, settings.ai_daily_budget):
                logger.warning(
                    "AI 日预算已达上限（%.4f），跳过自动打标 asset=%s",
                    settings.ai_daily_budget,
                    asset_id,
                )
                return
            auto_request_ai(session, asset_id)
    except Exception as exc:  # noqa: BLE001
        logger.warning("自动 AI 打标衔接失败 asset=%s: %s", asset_id, exc)
