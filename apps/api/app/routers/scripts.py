...
from clipmind_shared.models.enums import EXPORT_KIND_SCRIPT, ExportStatus
...
from app.services import (
    ...
    files,
    script_export_service,
    script_match_service,
    script_service,
)
...
async def create_csv_export(
    ...
    """CSV 导出（兼容旧端点）。"""
    export = await script_export_service.request_script_export(db, script_id, export_format="csv")
...
async def create_export(
    ...
    """多格式导出（csv/xlsx/json/markdown/printable），走真实 export-worker。"""
    export = await script_export_service.request_script_export(
        db, script_id, export_format=export_format
    )
    return _to_export_out(export)
...
async def download_csv_export(
    ...
    media_type = E.SCRIPT_EXPORT_CONTENT_TYPES.get(
        export.export_format, "application/octet-stream"
    )
    ...
    await export_center_service.record_download(db, EXPORT_KIND_SCRIPT, export_id)
    return response
