...
- **docker 容器内**真实 MiMo 曾调用失败：`error_code=unavailable`、无 HTTP 响应；诊断为
  Provider 端点解析到内网私有地址，与 Docker 自动分配的子网段重叠，容器误判为本网直连而
  `No route to host`（宿主机可达）。**属网络拓扑限制，非代码/鉴权/模型问题**；失败 run 已正确
  持久化为 `failed`（再次印证修复）。该网络问题的诊断与修复（可配置子网）见
  `docs/DOCKER_AI_NETWORKING.md`。CI 使用 `AI_PROVIDER=fake`，不受此限制影响。
