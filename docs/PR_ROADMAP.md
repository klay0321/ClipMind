...
- 引入 media worker（专用处理队列），承担拆镜头与派生文件任务。
- PySceneDetect 镜头检测 + 兜底切分策略（最短 / 最长镜头、首尾安全余量等可配置参数，遵循 PRD 7.4）。
- FFmpeg 派生：每镜头主关键帧 + 可选辅助关键帧、缩略图（统一尺寸、保持比例、WebP/JPEG）、代理视频（H.264、≤720P、可配置音频）、按需裁剪可下载片段。
- 派生文件统一写入命名卷 `clipmind-data:/app/data`（缩略图 / 关键帧 / 代理 / 片段分目录），不写入源目录。
- `Shot` 表与 `0002_*` 迁移（见 3.6）。
- 启用 `Asset.full_hash`（PR-01 预留字段）作为内容完整哈希补充判定。
- 镜头相关 API（在 PRD 13.3 / 13.4 范围内，例如 `GET /api/assets/{id}/shots`、`GET /api/shots`、`GET /api/shots/{id}`、镜头预览 / 片段导出接口）。

### 3.4 不在本 PR 范围（明确排除）

任何 AI 调用与打标、人工审核、向量与搜索、脚本匹配、CSV 导出。镜头的 AI 描述 / 标签 / 风险字段在本 PR 仅落库为空 / 预留，由 PR-03 填充。
...
- **新依赖**：PySceneDetect（镜头检测）+ media worker（新增处理队列与 worker 进程）+ FFmpeg（派生处理，PR-01 已具备运行时）。
- **新表**：`Shot`（`id, asset_id, sequence_no, start_time, end_time, duration, thumbnail_path, keyframe_paths, proxy_path, clip_path, description, quality_score, risk_level, review_status, created_at, updated_at` 等，遵循 PRD 12.5；其中 `description / quality_score / risk_level / review_status` 在本 PR 预留，PR-03 填充；`embedding` 列留待 PR-04）。
- **新迁移**：`0002_shots`（新增 `Shot` 表；启用 / 使用 `Asset.full_hash`）。只增不毁，可 `upgrade head` 与回退验证。

### 3.7 验收要点

- 对 `sample_media` 中样例视频执行处理后，`Shot` 记录的 `start_time / end_time / duration / sequence_no` 与实际镜头一致，时间码正确。
- 关键帧、缩略图、代理视频正确生成于 `clipmind-data`，浏览器可直接播放代理视频。
- 按需裁剪的片段可下载，入点 / 出点正确，源文件保持只读、未被改动。
- 处理失败可重试，不影响其他任务；media worker 队列可独立运行。
...
| PR-02 | `feat/shot-processing` | `0002_shots`（Shot；启用 full_hash） | media worker | 不启用 |
