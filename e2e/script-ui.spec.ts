...
test("脚本匹配工作台：创建→拆段→匹配→候选→选择锁定→剪辑清单→CSV", async ({ page }) => {
  ...
  // 新建脚本（创建表单在 Modal 内）→ 填写 → 创建 → 跳转工作台
  ...
  // 全脚本匹配 → 打开候选抽屉 → 第一段候选出现（真实 MiMo 逐段解析较慢，给足超时；CI Fake 很快）
  await page.getByTestId("match-all").click();
  // 候选镜头已改为按需抽屉（剪辑清单为主视图）：先打开抽屉再断言候选
  await page.getByTestId("open-candidates").click();
  ...
  // 剪辑清单已是工作台主视图（无二级 Tab）：关闭候选抽屉后主表可见
  ...
  // 导出统一入口：导出剪辑清单 ▾ → 选 CSV → 等待完成 → 下载按钮
  await page.getByTestId("toggle-export").click();
  await page.getByTestId("export-format-csv").click();
  await expect(page.getByTestId("multi-export-download")).toBeVisible({ timeout: 60_000 });
  await expect(page.getByTestId("multi-export-download")).toHaveAttribute("href", /\/api\/scripts\/\d+\/exports\/\d+\/download/);
  // eslint-disable-next-line no-console
